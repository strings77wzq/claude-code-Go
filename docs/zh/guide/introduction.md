...
claude-code-Go 是一个终端驱动的 AI 编程助手，完整实现了 Agent Loop（智能体循环），包含内置工具系统、三级权限管理、MCP（Model Context Protocol，Planned v0.3）协议支持以及 SSE（Server-Sent Events）流式输出。
...
4. **安全架构** — 三级权限模型、glob 规则匹配、Human-in-the-loop 审批
