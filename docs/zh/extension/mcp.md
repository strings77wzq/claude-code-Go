...
> **状态: 实验性功能 (v0.3 产品化中)** — MCP 客户端基础设施（传输层、JSON-RPC、适配器、管理器）已接入应用入口。MCP 服务器会从 `~/.config/go-code/mcp.json` 自动加载。每个 MCP 工具都需要显式审批，使用与内置工具相同的权限策略，并以 `mcp__{server}__{tool}` 命名。`go-code doctor --offline` 可在不启动服务器的情况下验证本地 MCP 配置。
...
MCP 是 go-code 中三种扩展机制之一，另外还有 Skills 和钩子。各自服务不同目的：

| 扩展 | 用途 | 工作方式 |
|-----------|---------|----------|
| **Skills** | 自定义 agent 行为 | 命名提示词注入系统提示词 |
...
在 `WorkspaceWrite` 或 `ReadOnly` 模式下，MCP 工具会经过与内置工具相同的权限策略。没有 allow rule 或会话审批时，MCP 工具调用必须先获得用户明确批准。会话 trace 会记录权限决策和 MCP 工具结果。
...
MCP 管理器会在启动时加载该文件；文件不存在时不会中断启动。
...
- `[SKIP] mcp`：`~/.config/go-code/mcp.json` 不存在。
- `[FAIL] mcp`：配置文件存在但不可读或无法解析。
- `[PASS] mcp`：配置文件可读，并包含服务器条目。

启动行为仍然是非致命的：如果某个 MCP 服务器无法启动或无法列出工具，go-code 会记录失败并继续使用内置工具。

## 当前限制

- 已实现 stdio 传输；其他 MCP transport 尚未产品化。
- MCP 服务器启动是 best-effort，目前除日志和 doctor 配置诊断外，还没有更详细的 UI。
- MCP 工具被保守地视为外部副作用工具，因此始终需要权限审批。
- CI 覆盖使用本地 fixture，不依赖真实外部 MCP 服务器或凭据。
