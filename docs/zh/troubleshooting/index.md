...
go-code 默认以 `WorkspaceWrite` 启动：读取工作区文件通常直接允许；写文件、编辑文件、执行 Bash、网络访问或其他副作用操作会先请求确认。

审批提示支持：

- `y` / `yes`：仅允许本次操作。
- `n` / `no`：拒绝本次操作，工具不会执行，agent 会收到错误形式的 tool result。
- `a` / `always`：在当前会话中记住同类操作，后续匹配操作不再重复询问。

如果读写被阻止，优先检查路径是否解析到了工作区外、目标是否为二进制文件、文件是否过大，或 Bash 命令是否包含删除、提权、系统目录重定向等危险模式。权限审计会记录工具名、决策、摘要和时间戳，但不会记录完整敏感载荷。
