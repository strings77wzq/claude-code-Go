...
这个状态代表模型对用户请求已完成的置信度。驾驭系统不会质疑这个决定 — 它信任模型的评估并返回结果。
...
2. **并行执行**：多个工具请求并行执行。模型可以同时请求几个操作，驾驭系统协调它们的执行。
...
当 `stop_reason` 为 `max_tokens` 时，响应因达到最大 token 限制而被截断。这不是失败 — 它是一个边界条件。驾驭系统返回部分响应并附带警告，允许用户决定是否继续。
...
对话历史不仅仅是日志 — 它是允许模型理解正在进行的任务的**上下文**。管理这个历史是驾驭系统最关键的职责之一。
...
API 客户端增量接收 token，并为每个 token 增量调用回调。驾驭系统使用此回调来：
- 将文本输出流式传输到终端
- 在工具执行期间显示进度指示器
- 为用户提供实时反馈
