...
想象一架飞机：**飞行员**提供判断力、决策能力和态势感知 — 读取仪表、评估天气条件、决定行动方案。**自动驾驶仪**提供可靠性 — 保持高度、遵循飞行计划、防止飞机失速或超速、在安全参数范围内处理气流。
...
模型（飞行员）决定*做什么* — 要编辑哪些文件、要运行哪些命令、要尝试哪种方法。Harness（自动驾驶仪）确保执行保持在安全边界内 — 工具在超时内完成、输出适合上下文、操作符合策略允许。
...
**我们选择这个而非基础框架的原因**：许多"代理框架"只提供代理循环，将安全、持久化和权限留给用户。这适用于实验但不适用于生产使用。本系统是一个包含所有可靠性功能的代理*产品*。
