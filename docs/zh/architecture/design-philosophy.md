---
title: 设计理念
description: 深入探讨智能/可靠性分离 —— 模型负责什么 vs. Harness 保证什么
---
...
**模型提供智能，Harness 提供可靠性** — 这不是一句营销口号。它是塑造 go-code 每一行代码的架构约束。
...
go-code 位于这个边界上。模型处理认知工作 — 理解用户想要什么，决定调用哪些工具，解释结果，规划下一步。Harness 处理可靠性工作 — 确保操作在边界内执行，防止资源耗尽，维护会话状态，并强制执行安全边界。

这种分离不仅仅是方便的。它是必要的，因为**模型可能会出错，但 Harness 绝不能失败**。
...
Agent Loop 是迭代的。在每个工具执行后，模型必须决定：继续下一步，尝试不同的方法，要求澄清，或宣布任务完成。这是一个需要理解整体目标和当前进度的规划和推理任务。
...
**go-code 的立场**：模型直接采取行动，Harness 通过权限检查和超时保护确保安全。

### 脚本运行器范式

在脚本运行器中，预定义脚本在没有 LLM 参与的情况下执行。LLM 生成脚本参数但无法控制执行流程。

**go-code 的立场**：模型控制执行流程，选择调用哪些工具以及按什么顺序。Harness 确保执行保持在边界内。
...
许多"代理框架"只提供 Agent Loop，将安全、持久化和权限留给用户。这适用于实验但不适用于生产使用。
...
- 如果一个能力需要**保证安全** → Harness 责任
- 如果一个能力需要**资源管理** → Harness 责任
- 如果一个能力需要**跨会话持久化** → Harness 责任
...
- [Agent Loop](agent-loop.md) — 状态机机制和执行周期
