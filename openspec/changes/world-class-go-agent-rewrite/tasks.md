...
- [ ] 8.1 Add configuration loading for MCP servers and register namespaced MCP tools through the existing registry.
- [ ] 8.2 Apply permission policy consistently to MCP tool execution.
- [ ] 8.3 Add tests for MCP server load failure, tool registration, and permission-gated MCP execution.
- [ ] 8.4 Document hook lifecycle guarantees and add tests for pre-hook block and post-hook error behavior.
- [ ] 8.5 Gate LSP commands/tools behind configured healthy LSP servers and add unavailable-state behavior.
- [ ] 8.6 Validate skill files, list valid skills, report invalid skills non-fatally, and document the skill format.
- [ ] 8.7 Update extension docs for MCP, hooks, LSP, and skills.

## 9. Documentation Product Experience

- [ ] 9.1 Rewrite README around verified quick start, doctor, first prompt, core features, and honest status.
- [ ] 9.2 Restructure Chinese docs into install, configure, doctor, usage, troubleshooting, architecture, extension, parity, roadmap, and contribution paths.
- [ ] 9.3 Mirror critical Chinese docs in English or mark translation gaps explicitly.
- [ ] 9.4 Replace aspirational showcase/testimonials with real examples, reproducible demos, or clearly labeled placeholders removed from public pages.
- [ ] 9.5 Rewrite benchmark docs with methodology, commands, environment, date, and reproduction steps.
- [ ] 9.6 Add architecture docs that map runtime flows to actual packages and interfaces.
- [ ] 9.7 Add troubleshooting pages for API auth, provider routing, permission denial, session resume, command not found, and docs build issues.
- [ ] 9.8 Run docs build and fix broken navigation or dead internal links.

## 10. Open Source And Release Readiness

- [ ] 10.1 Ensure CI runs formatting, vet/static checks, Go tests, harness tests, docs build, and OpenSpec validation.
- [ ] 10.2 Update issue templates, pull request template, and contribution guide with required test evidence.
- [ ] 10.3 Add first-good-issue guidance and package-boundary notes for contributors.
- [ ] 10.4 Review release workflow for platform binaries, checksums, release notes, and install instructions.
- [ ] 10.5 Add changelog discipline for user-visible changes.
- [ ] 10.6 Update roadmap with now, next, later, and not-planned sections.
- [ ] 10.7 Add ownership/affiliation disclaimer where the project references Claude Code or Anthropic.

## 11. Final Verification

- [ ] 11.1 Run `go test ./...` and record results.
- [ ] 11.2 Run full repository test command and record results.
- [ ] 11.3 Run parity harness and record scenario summary.
- [ ] 11.4 Run docs build and record results.
- [ ] 11.5 Run `openspec validate world-class-go-agent-rewrite --strict`.
- [ ] 11.6 Run `go-code doctor` against a documented local configuration.
- [ ] 11.7 Update `PARITY.md`, roadmap, and release notes with final status and remaining gaps.
