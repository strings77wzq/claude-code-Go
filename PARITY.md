...
| Built-in file tools | verified | `internal/tool/builtin/` 11 test files, 70.2% coverage (read, write, edit, bash, glob, grep, tree, webfetch, diff, todo, notebook, validate) | executeCell testing requires python/node runtime; some error paths not covered. |
| Bash tool | verified | `internal/tool/builtin/bash_test.go` covers timeout/truncation/destructive blocking; harness covers bash tool roundtrip and denial. Semantic validator: `internal/permission/bash_semantic_test.go` covers classification, destructive detection, path validation, pipes/chains. | Interactive approval UX still needs TUI polish. |
| Permission prompts | verified | `internal/permission/policy_test.go`, `rules_test.go`, `bash_semantic_test.go` cover rules/policy/semantic validation; agent tests cover deny/remembered approvals; harness covers denial without executing. | Approval mode flags are still not exposed as CLI options. |
